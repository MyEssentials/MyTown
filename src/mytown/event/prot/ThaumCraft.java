...
public class ThaumCraft extends ProtBase {
    ...
    public String update(Entity e) throws Exception {
        if (clAlumentum.isInstance(e)) {
            ...
            if (!thrower.canInteract(dim, x - explosionRadius, y, z - explosionRadius, "build") || !thrower.canInteract(dim, x - explosionRadius, y, z + explosionRadius, "build") || !thrower.canInteract(dim, x + explosionRadius, y, z - explosionRadius, "build")
                    || !thrower.canInteract(dim, x + explosionRadius, y, z + explosionRadius, "build")) {
            ...
            if (!thrower.canInteract(dim, x - radius, y - radius, y + radius, z - radius, "build") || !thrower.canInteract(dim, x - radius, y - radius, y + radius, z + radius, "build") || !thrower.canInteract(dim, x + radius, y - radius, y + radius, z - radius, "build") || !thrower.canInteract(dim, x + radius, y - radius, y + radius, z + radius, "build")) {
            ...
            if (!res.canInteract(dim, x - radius, y - radius, y + radius, z - radius, "build") || !res.canInteract(dim, x - radius, y - radius, y + radius, z + radius, "build") || !res.canInteract(dim, x + radius, y - radius, y + radius, z - radius, "build") || !res.canInteract(dim, x + radius, y - radius, y + radius, z + radius, "build")) {
        ...
        if (clItemWandCasting.isInstance(tool)){
            ...
            if (focusName.equals("thaumcraft.common.items.wands.foci.ItemFocusFire")){
                ...
                if (pos != null && pos.typeOfHit == EnumMovingObjectType.TILE) {
                    if (!res.canInteract(res.onlinePlayer.dimension, pos.blockX, pos.blockY, pos.blockZ, "build")) {
                        return "Cannot build here";
                    }
                }
            ...
            //Thaumic Tinkerer Foci
                ...
                int z = pos.blockZ;
                int dim = res.onlinePlayer.dimension;
                if (!res.canInteract(dim, x , y, y, z, "build")) {
                    return "Cannot build here";
                }
            }
        }
